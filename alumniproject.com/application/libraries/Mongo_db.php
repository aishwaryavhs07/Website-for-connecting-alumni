...
class Mongo_db {
	...
	*
	*	$conds will be such format : $conds[]=array('email_address' => "$emails");
	*
	*	It will not search 'like' condition text
	*
	*	@usage : $this->mongo_db->search('$conds')->get('table_name');
	*	
	*	returns number max('foo') in foobar table
	*/
	public function search($wheres, $value = null)
	{
		if($wheres == ''){
			$newarray = array();
		}else{
			foreach($wheres as $narray => $value){
				$newarray = $value;
			}
		}
		if (is_array($newarray))
		{
			foreach ($newarray as $wh => $val)
			{
				$this->wheres[$wh] = $val;
			}
		}
	...
	*	Get the documents where the value of a $field may be something else
	*
	*	@usage : $this->mongo_db->automatic_inc('foobar','foo');
	*	
	*	returns number max('foo') in foobar table
	*/
	public function automatic_inc($collection = "", $where)
	{
		$newarray = $this->select(array($where));
		if (is_array($newarray))
		{
			foreach ($newarray as $wh => $val)
			{
		...
		$new = array();
		$userid_array = $this->get($collection);
		foreach($userid_array as $array){
			$new[] = $array->$where;
		}
		if(count($new) == 0){
			return 1;
	...
	*/
	...
	public function or_where_gte($wheres = array(),$field = "", $x)
	{
		if (count($wheres) > 0)
		{
			if ( ! isset($this->wheres['$or']) || ! is_array($this->wheres['$or']))
	...
	*/
	...
	public function slike($field = array(), $value = "", $flags = "i", $enable_start_wildcard = TRUE, $enable_end_wildcard = TRUE)
	 {
	 	//$field = (string) trim($field);
	 	//$this->_where_init($field);
	 	$value = (string) trim($value);
	 	...
	 	foreach ($field as $fields) {
		 	$regex = "/$value/$flags";
		 	$this->wheres[$fields] = new MongoRegex($regex);
	 	}
	 	return ($this);
	 }
	...
	*	@usage: $this->mongo_db->(array('blog_id'=>123))->pop(array('comments', 'viewed_by'))->update('blog_posts');
	*/
	...
	*/
	...
	{
		if ((! is_array($data)) and (! is_object($data))) return array(); 
		$result = array();
		$data = (array) $data;
		foreach ($data as $key => $value) {
		    if (is_object($value)) $value = (array) $value;
		    if (is_array($value)) {
		    $result[$key] = $this->row_array($value);
		    }else{
		        $result[$key] = $value;
		    }
		}
